...
if (FALSE) {
  ...
  saveRDS(fit_rstan, "fit_rstan_physio_v2_5cls.rds")
}

# read in GLDA result
fit_rstan <- readRDS("fit_rstan_physio_v2_5cls.rds")
...
rstan::traceplot(fit_rstan, pars = paste('mu[', 1:5, ',1]', sep = ''), inc_warmup = T)
...
ggplot(data.frame(index = rep(1:5,5), variable = c(rep("K1",5), rep("K2", 5), rep("K3",5), rep("K4",5), rep("K5",5))) %>% 
         mutate(mu = c(mu[1,], mu[2,], mu[3,], mu[4,], mu[5,])), aes(x = as.numeric(index), color = as.factor(variable))) + theme_bw() +
  ...
  scale_color_discrete(name = "cluster", limits = c("K1", "K2", "K3", "K4", "K5")) +
...
data.scaled.post <- data.scaled %>% mutate(m = as.integer(factor(data.scaled$pid))) %>% 
  ...
  mutate(post.sum = post.1 + post.2 + post.3 + post.4 + post.5) %>% 
  mutate(post.1 = post.1/post.sum, post.2 = post.2/post.sum, post.3 = post.3/post.sum, 
         post.4 = post.4/post.sum, post.5 = post.5/post.sum)

data.scaled.post$glda.cl <- apply(data.scaled.post[, c("post.1", "post.2", "post.3", "post.4", "post.5")], 1, 
                                  function(x)  factor(c("K1", "K2", "K3", "K4", "K5"))[which.max(x)])
data.scaled.post

# calculate BIC of the 5-class GLDA model (manually)
K <- 5
V <- 5
M <- stan_data$M
...
bic.5cls <- d*log(N) - 2*logli
bic.5cls

# explore correlations between physio variables and class posteriors over all observations
cor(data.scaled.post[,c("hr", "rmssd", "pep", "resp_rate", "resp_amp")], 
    data.scaled.post[, c("post.1", "post.2", "post.3", "post.4", "post.5")])

# try plotting an example participants' momentary cluster membership transition
pid.example <- data.scaled.post %>% select(pid, segment, post.1:post.5, glda.cl) %>% filter(pid == "c001")  
ggplot(pid.example, aes(x = segment, y = as.integer(glda.cl))) + theme_bw() +
  geom_point(size = 0.5) +
  geom_path() +
  scale_x_continuous(breaks = seq(0, nrow(pid.example), 25)) +
  scale_y_continuous(name = "GLDA-inferred discrete state", breaks = 1:5, labels = c("K1", "K2", "K3", "K4", "K5")) +
...
groupdata.glda <- data.scaled.post %>% mutate(time = segment-1) %>% group_by(pid) %>% 
  ...
  cbind(data.frame(theta1 = theta[,1], theta2 = theta[,2], theta3 = theta[,3],
                   theta4 = theta[,4], theta5 = theta[,5])) # the participant order of theta matches that of groupdata.glda after the summarize
...
groupdata.glda.cor <- lapply(unique(data.scaled.post$pid), function(p) {
  cor(data.scaled.post[which(data.scaled.post$pid == p), c("hr", "rmssd", "pep", "resp_rate", "resp_amp")], 
      data.scaled.post[which(data.scaled.post$pid == p), c("post.1", "post.2", "post.3", "post.4", "post.5")])
})

apply(array(unlist(groupdata.glda.cor), c(5, 5, 166)), c(1,2), mean) 
apply(array(unlist(groupdata.glda.cor), c(5, 5, 166)), c(1,2), sd) 

# check correlations between personalized class weights (theta) and realized class frequencies; notice the diagonal is all >0.95
cor(groupdata.glda[,c("cl1rate", "cl2rate", "cl3rate", "cl4rate", "cl5rate")], 
    groupdata.glda[,c("theta1", "theta2", "theta3", "theta4", "theta5")])

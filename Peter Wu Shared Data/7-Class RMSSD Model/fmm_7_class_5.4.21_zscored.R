...
names(dfList) <- c('p002', 'p003', 'p004', 'p006', 'p007', 'p008', 'p009', 'p010', 'p012', 'p014',
                   ...
                   'p067', 'p069', 'p102', 'p118', 'p119', 'p120', 'p124', 'p126', 'p134',
                   'p153', 'p164', 'p174', 'p177', 'p178', 'p181', 'p182', 'p184', 'p188', 'p192',
                   'p194', 'p197', 'c001', 'c002', 'c004', 'c005', 'c009', 'c010', 'c011', 'c012',
                   'c013', 'c014', 'c015', 'c018', 'c019', 'c020', 'c021', 'c023', 'c024', 'c025',
                   'c026', 'c027', 'c028', 'c029', 'c031', 'c032', 'c033', 'c034', 'c036', 'c038',
                   'c039', 'c041', 'c044', 'c045', 'c046', 'c047', 'c048', 'c050', 'c052', 'c053',
                   'c054', 'c055', 'c057', 'c058', 'c061', 'c063', 'c065', 'c066', 'c067', 'c068',
                   'c070', 'c071', 'c072')
...
eachLCA = lapply(dfList, function(x) Mclust(scale(na.omit(x[,c(2, 11, 17, 14, 15, 5, 6)])), modelName = c('EII', 'VII', 'EEI', 'VEI', 'EVI', 'VVI'))) 
...
lcaSummary=data.frame(id=character(222),g=double(222), stringsAsFactors = FALSE) ##making an empty data frame with two columns, specifying the type of variable for each column (double=numeric)
...
median(lcaSummary$g) #6
sd(lcaSummary$g) #2.97
...
mc=data.frame(id=idXg, class = classes, hr=double(sum(lcaSummary$g)), 
              ...
              sv=double(sum(lcaSummary$g)), co=double(sum(lcaSummary$g)), 
...
for (i in 1:nrow(mc)){
  mc[i,3:9] = eachLCA[[paste(mc$id[i])]]$parameters$mean[,as.numeric(paste(mc$class[i]))]
}

#run between-subjects LCA
#  change G to whatever is appropriate for your data, make sure you also index same columns
mod = Mclust(mc[,c(3:9)], modelName = c('EII', 'VII', 'EEI', 'VEI', 'EVI', 'VVI'))
mod = Mclust(mc[,c(3:9)], G = 1:20, modelName = c('EII', 'VII', 'EEI', 'VEI', 'EVI', 'VVI'))
summary(mod)
...
BIC <- mclustBIC(mc[,c(3:9)], G = 1:20, modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(BIC) 
plot(BIC)

ICL <- mclustICL(mc[,c(3:9)], G = 1:20, modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(ICL) 
plot(ICL)

#Entropy plot
output <- clustCombi(data = mc[,3:9], G = 1:20, modelNames = "VVI") 
...
blrt=mclustBootstrapLRT(mc[,c(3:14)], maxG=2, modelName = "EVI") 
blrt 

#save best model as an object
mod = Mclust(mc[,c(3:9)], G=7, modelName = c('VVI'))
summary(mod)
...
means$Class=dplyr::recode(means$Class, X1 = "1", X2 = "2", X3 = "3", X4 = "4", X5 = "5", X6 = "6", X7 = "7")

#Plot profiles
pdf('C:/Users/Aaron/Dropbox/Lab/Phys Data/FMM Project/FMM_7classZ_RMSSD.pdf')
means%>%
  ggplot(aes(Variable, Mean, group = Class, color = Class)) +
  geom_point(size = 2.25) +
  geom_line(size = 1.25) +
  scale_x_discrete(limits = c("hr", "rsa", "pep", "sv", "co", "rate", "amp")) +
...
nalist = lapply(dfList, function(x) na.omit(x[,c(2, 11, 17, 14, 15, 5, 6)]))
...
for (i in 1:nrow(groupdata)){
  ...
  groupdata$nbclasses[i] =sum(groupdata$cl1[i]+groupdata$cl2[i]+groupdata$cl3[i]+groupdata$cl4[i]+
                                groupdata$cl5[i]+groupdata$cl6[i]+groupdata$cl7[i])
}
...
cor(nalist$p002[,c(1:7, 10:17)])
as.vector(cor(nalist$p002[,c(1:7, 10:17)]))[c(2,3,18)]
as.vector(cor(nalist$p002[,c(1:7, 10:17)])[1:7,15])
as.vector(cor(nalist$p002[,c(1:7, 10:17)])[12,1])

#Add correlations and means to groupdata
for (i in 1:nrow(groupdata)){
  groupdata[i, 24:26] = as.vector(cor(nalist[[paste(groupdata$id[i])]][,c(1:7, 10:17)])[c(2,3,18)])
  groupdata[i, 27:33] = as.vector(cor(nalist[[paste(groupdata$id[i])]][,c(1:7, 10:17)])[1:7,15])
  groupdata[i, 34] = mean(nalist[[paste(groupdata$id[i])]]$hr, na.rm = TRUE)
  groupdata[i, 35] = mean(nalist[[paste(groupdata$id[i])]]$rmssd, na.rm = TRUE)
  groupdata[i, 36] = mean(nalist[[paste(groupdata$id[i])]]$pep, na.rm = TRUE)
  groupdata[i, 37] = mean(nalist[[paste(groupdata$id[i])]]$sv, na.rm = TRUE)
  groupdata[i, 38] = mean(nalist[[paste(groupdata$id[i])]]$co, na.rm = TRUE)
  groupdata[i, 39] = mean(nalist[[paste(groupdata$id[i])]]$resp_rate, na.rm = TRUE)
  groupdata[i, 40] = mean(nalist[[paste(groupdata$id[i])]]$resp_amp, na.rm = TRUE)
  groupdata[i, 41] = mssd(nalist[[paste(groupdata$id[i])]]$hr)
  groupdata[i, 42] = ifelse(sd(nalist[[paste(groupdata$id[i])]]$hr, na.rm = TRUE)>0, ar(nalist[[paste(groupdata$id[i])]]$hr, max.order = 1)$ar, NA)
  groupdata[i, 43] = sd(nalist[[paste(groupdata$id[i])]]$hr, na.rm = TRUE)
  groupdata[i, 44] = quantile(nalist[[paste(groupdata$id[i])]]$hr, 0.80) - quantile(nalist[[paste(groupdata$id[i])]]$hr, 0.20)
  groupdata[i, 45] = as.vector(cor(nalist[[paste(groupdata$id[i])]][,c(1:7, 10:17)])[12,1])
  groupdata[i, 46] = as.vector(cor(nalist[[paste(groupdata$id[i])]][,c(1:7, 10:17)])[14,1])
}

colnames(groupdata)[24:46] = c('hrrsa', 'hrpep', 'rsapep',
                               'hrslope', 'rsaslope', 'pepslope', 'svslope', 'coslope', 'rrslope', 'ampslope',
                               'hr', 'rsa', 'pep', 'sv', 'co', 'rate', 'amp',
                               'hrmssd', 'hrar', 'sdhr', 'hrrange',
                               'cl5hr', 'cl7hr')
...
dat = datx[, c(1:4, 6:19, 21:24, 29:31, 35,36, 45:49, 77:121)]
View(dat)

...
dati <- InsertRow(dati, NewRow=new, RowNum = 6)
dati <- InsertRow(dati, NewRow=new, RowNum = 10)
dati <- InsertRow(dati, NewRow=new, RowNum = 163)
dati$start

#Code days of the week
dati$day <- rep(1:7, each=4, length.out=nrow(dati))
# 1 = Wednesday
dati$mon=ifelse(dati$day==6,1,0)
dati$tues=ifelse(dati$day==7,1,0)
dati$wed=ifelse(dati$day==1,1,0)
dati$thur=ifelse(dati$day==2,1,0)
dati$fri=ifelse(dati$day==3,1,0)
dati$sat=ifelse(dati$day==4,1,0)
dati$sun=ifelse(dati$day==5,1,0)
...
summary(Mclust(clustdat, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #contains empty class
summary(Mclust(clustdat, G=5, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #contains empty class
summary(Mclust(clustdat, G=4, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #contains empty class
summary(Mclust(clustdat, G=3, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #contains empty class
summary(Mclust(clustdat, G=2, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #contains empty class

#Verify model selection via BIC, ICL, and BLRT
BIC <- mclustBIC(clustdat,G=c(1,2,3,4), modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(BIC)
ICL <- mclustICL(clustdat,G=c(1,2,3,4), modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=2, modelName = "EEI")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 2, modelName = "EEI")
summary(mod, parameters=T)

#Save classification data
x=as.data.frame(cbind(mod$z,mod$classification,mod$uncertainty))
colnames(x) <- c("prob1", "prob2", "class", "uncertain")
View(x)
...
means$Class=recode(means$Class, X1 = "1", X2 = "2")
...
#Run mVAR
set.seed(1234)
var2=mvar(x[,5:6], type=c("c", "c"), level=c(2, 2), lamdaSel="EBIC",
...
set.seed(1234)
regL2=cv.glmnet(matL1.1,trainL0$class2,family='binomial',standardize=T,alpha=.5)

...
dati <- InsertRow(dati, NewRow=new, RowNum = 73)
dati <- InsertRow(dati, NewRow=new, RowNum = 83)
dati$start
...
summary(Mclust(clustdat, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #6 components, 2 with < 10%
summary(Mclust(clustdat, G=5, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #1 with < 10%
...
length(which(x$class==1))/length(x$class) #27.78%
length(which(x$class==2))/length(x$class) #23.33%
length(which(x$class==3))/length(x$class) #17.78%
length(which(x$class==4))/length(x$class) #31.11%
...
set.seed(1234)
regL3=cv.glmnet(matL1.1,trainL0$class3,family='binomial',standardize=T,alpha=.5)

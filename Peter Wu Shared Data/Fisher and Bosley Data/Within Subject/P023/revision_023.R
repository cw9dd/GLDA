...
dati <- InsertRow(dati, NewRow=new, RowNum = 27)
dati <- InsertRow(dati, NewRow=new, RowNum = 55)
dati <- InsertRow(dati, NewRow=new, RowNum = 99)
dati$start
...
summary(Mclust(clustdat, modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))) #5 components, 1 with < 10%
#summary(Mclust(clustdat, G=4, modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))) 
#summary(Mclust(clustdat, G=3, modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))) 
#summary(Mclust(clustdat, G=2, modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI")))

#Verify model selection via BIC, ICL, and BLRT
BIC <- mclustBIC(clustdat,G=c(1,2,3,4), modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(BIC)
ICL <- mclustICL(clustdat,G=c(1,2,3,4), modelName = c("EII", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=3, modelName = "VVI")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 4, modelName = "VVI")
...
#  1         2         3         4 
#0.3581311 0.1266471 0.4517918 0.0634300 
...
means$Class=recode(means$Class, X1 = "1", X2 = "2", X2 = "3", X2 = "4")
...
set.seed(1234)
regL1=cv.glmnet(matL1.1,trainL0$class1,family='binomial',standardize=T,alpha=.001)
...
#        threshold specificity sensitivity 
#best   -0.1409560   0.5909091   0.8181818
#best.1 -0.1133175   0.8636364   0.5454545
...
set.seed(1234)
regL4=cv.glmnet(matL1.1,trainL0$class4,family='binomial',standardize=T,alpha=.05)

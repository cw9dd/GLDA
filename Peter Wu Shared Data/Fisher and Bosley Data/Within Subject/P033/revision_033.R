...
dati <- InsertRow(dati, NewRow=new, RowNum = 107)
dati$start

#Code days of the week
dati$day <- rep(1:7, each=4, length.out=nrow(dati))
# 1 = Thursday
dati$mon=ifelse(dati$day==5,1,0)
dati$tues=ifelse(dati$day==6,1,0)
dati$wed=ifelse(dati$day==7,1,0)
dati$thur=ifelse(dati$day==1,1,0)
dati$fri=ifelse(dati$day==2,1,0)
dati$sat=ifelse(dati$day==3,1,0)
dati$sun=ifelse(dati$day==4,1,0)
...
summary(Mclust(clustdat, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #3classes w/ n=3
summary(Mclust(clustdat, G=6, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #1class w/ n=5
summary(Mclust(clustdat, G=5, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))) #1class w/ n=5
summary(Mclust(clustdat, G=4, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI")))
...
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=4, modelName = "VEI")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 4, modelName = "VEI")
summary(mod, parameters=T)

#Save classification data
x=as.data.frame(cbind(mod$z,mod$classification,mod$uncertainty))
colnames(x) <- c("prob1", "prob2","prob3", "prob4", "class", "uncertain")
View(x)
...
#  1         2         3         4 
#0.2317731 0.2787398 0.3384940 0.1509932 
...
means$Class=recode(means$Class, X1 = "1", X2 = "2", X3 = "3", X4 = "4")
...
#Run mVAR
set.seed(1234)
var4=mvar(x[,7:10], type=c("c", "c", "c", "c"), level=c(2, 2, 2, 2), lamdaSel="EBIC",
          lags=1, beepvar=beepvar, dayvar=dayvar, alphaSeq=1, binarySign = TRUE, overparameterize = TRUE)

qgraph(t(var4$wadj[, , 1]),edge.color = t(var4$edgecolor[, , 1]))
...
#best   -1.888888   0.8095238   0.7142857
#best.1 -1.153531   0.9523810   0.5714286
...
#      threshold specificity sensitivity
#best -0.7743224   0.5714286   0.7142857
...
set.seed(1234)
regL3=cv.glmnet(matL1.1,trainL0$class3,family='binomial',standardize=T,alpha=.05)

...
dati <- InsertRow(dati, NewRow=new, RowNum = 78)
dati <- InsertRow(dati, NewRow=new, RowNum = 80)
dati <- InsertRow(dati, NewRow=new, RowNum = 132)
dati=dati[-133,]
dati$start
...
#summary(Mclust(clustdat, G=1:2, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI")))
...
set.seed(1234)
regL2=cv.glmnet(matL1.1,trainL0$class2,family='binomial',standardize=T,alpha=.05)
...
#      threshold specificity sensitivity
#best   -1.2779494        0.65         1.0
#best   -0.5303353        0.75         0.9
...
set.seed(1234)
regL3=cv.glmnet(matL1.1,trainL0$class3,family='binomial',standardize=T,alpha=.0004)

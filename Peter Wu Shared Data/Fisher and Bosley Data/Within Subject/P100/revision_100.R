...
dati <- InsertRow(dati, NewRow=new, RowNum = 69)
dati <- InsertRow(dati, NewRow=new, RowNum = 140)
dati=dati[-141,]
dati$start
...
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=5, modelName = "EEI")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 4, modelName = "EEI") #Too few observations in classes 2 and 4 for train/test (5-class solution)
...
#        1         2         3         4
#0.2434141 0.1785725 0.1642549 0.4137585 
...
set.seed(1234)
regL4=cv.glmnet(matL1.1,trainL0$class4,family='binomial',standardize=T,alpha=.0003)

...
dati <- InsertRow(dati, NewRow=new, RowNum = 37)
dati <- InsertRow(dati, NewRow=new, RowNum = 38)
...
# 1 = Monday
dati$mon=ifelse(dati$day==1,1,0)
dati$tues=ifelse(dati$day==2,1,0)
dati$wed=ifelse(dati$day==3,1,0)
dati$thur=ifelse(dati$day==4,1,0)
dati$fri=ifelse(dati$day==5,1,0)
dati$sat=ifelse(dati$day==6,1,0)
dati$sun=ifelse(dati$day==7,1,0)
...
BIC <- mclustBIC(clustdat,G=1:5, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(BIC)
ICL <- mclustICL(clustdat,G=1:5, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=4, modelName = "EEI")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 4, modelName = "EEI") 
...
#        1         2         3        4
#0.2878683 0.1791475 0.3270259 0.2059583 
...
#best -0.329959   0.8181818   0.9090909

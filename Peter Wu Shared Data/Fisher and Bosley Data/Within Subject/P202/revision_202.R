...
dati <- InsertRow(dati, NewRow=new, RowNum = 87)
dati <- InsertRow(dati, NewRow=new, RowNum = 97)
dati <- InsertRow(dati, NewRow=new, RowNum = 109)
dati$start
...
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=4, modelName = "VII")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 4, modelName = "VII") 
...
#        1         2         3         4
#0.1778038 0.4459656 0.2522665 0.1239642
...
set.seed(1234)
regL2=cv.glmnet(matL1.1,trainL0$class2,family='binomial',standardize=T,alpha=.0004)
...
set.seed(1234)
regL3=cv.glmnet(matL1.1,trainL0$class3,family='binomial',standardize=T,alpha=.02)
...
set.seed(1234)
regL4=cv.glmnet(matL1.1,trainL0$class4,family='binomial',standardize=T,alpha=.1)

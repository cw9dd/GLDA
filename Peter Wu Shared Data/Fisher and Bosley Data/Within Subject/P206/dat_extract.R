...
means[22]=mean(data[,27],na.rm=TRUE)

#Standard deviations for all EMA variables
sds=as.vector(numeric(22))
...
sds[22]=sd(data[,27],na.rm=TRUE)

#Normalized means (mean/max mean)
normmean=as.vector(numeric(22))
...
normmean[22]=mean(data[,27],na.rm=TRUE)/max(means)

#Normalized SD (SD/max SD)
normsd=as.vector(numeric(22))
...
normsd[22]=sd(data[,27],na.rm=TRUE)/max(sds)
...
list=c("energetic","enthusiastic","content","irritable","restless","worried",
       ...
       "avoid_act","procrast","avoid_people")

sumdat=as.data.frame(cbind(list, means, normmean, sds, normsd))
write.csv(sumdat,'summary_data_206.csv',row.names = FALSE)

...
#Rename
colnames(data) <- c("start","finish","energetic","enthusiastic","content","irritable","restless","worried","guilty","afraid","anhedonia","angry","hopeless","down","positive","fatigue","tension","concentrate","accepted","threatened","ruminate","avoid_act","procrast","hours","difficult","unsatisfy","avoid_people")
...
dati <- InsertRow(dati, NewRow=new, RowNum = 56)
dati <- InsertRow(dati, NewRow=new, RowNum = 66)
dati <- InsertRow(dati, NewRow=new, RowNum = 76)
dati <- InsertRow(dati, NewRow=new, RowNum = 126)
dati <- InsertRow(dati, NewRow=new, RowNum = 131)
dati$start

#Code days of the week
dati$day <- rep(1:7, each=4, length.out=nrow(dati))
# 1 = Saturday
dati$mon=ifelse(dati$day==3,1,0)
dati$tues=ifelse(dati$day==4,1,0)
dati$wed=ifelse(dati$day==5,1,0)
dati$thur=ifelse(dati$day==6,1,0)
dati$fri=ifelse(dati$day==7,1,0)
dati$sat=ifelse(dati$day==1,1,0)
dati$sun=ifelse(dati$day==2,1,0)
...
datx$filter=ifelse(!complete.cases(datx[,c(3:20)]),1,0)
...
clustdat <- as.data.frame(scale(daty[, c(6,12,10,8,21,14,13,11,22,27)]))
...
summary(Mclust(clustdat, G=1:6, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI")))
...
BIC <- mclustBIC(clustdat,G=1:3, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(BIC)
ICL <- mclustICL(clustdat,G=1:3, modelName = c("EEI", "VII", "EEI", "VEI", "EVI", "VVI"))
summary(ICL)
...
datyL0[,c(3:23,27,29:30,44:52,56)] <- sapply(datyL0[,c(3:23,27,29:30,44:52,56)], as.character)
datyL0[,c(3:23,27,29:30,44:52,56)] <- sapply(datyL0[,c(3:23,27,29:30,44:52,56)], as.numeric)
datyL1[,c(3:23,27,29:30,44:52,56)] <- sapply(datyL1[,c(3:23,27,29:30,44:52,56)], as.character)
datyL1[,c(3:23,27,29:30,44:52,56)] <- sapply(datyL1[,c(3:23,27,29:30,44:52,56)], as.numeric)
...
matL1.1 = data.matrix(cbind(trainL1[,c(3:23,27)],trainL0[,c(32:38,41:52)]))
matL1.2 = data.matrix(cbind(testL1[,c(3:23,27)],testL0[,c(32:38,41:52)]))
#Psychosocial variables + time variables + uncertainty
matL2.1 = data.matrix(cbind(trainL1[,c(3:23,27)],trainL0[,c(32:38,41:52,56)]))
matL2.2 = data.matrix(cbind(testL1[,c(3:23,27)],testL0[,c(32:38,41:52,56)]))
...
set.seed(1234)
regL1=cv.glmnet(matL1.1,trainL0$class1,family='binomial',standardize=T,alpha=.5)
...
set.seed(1234)
regL2=cv.glmnet(matL1.1,trainL0$class2,family='binomial',standardize=T,alpha=.001)
...
#  threshold specificity sensitivity
#best   -0.6046856   0.4074074   0.7777778
#best.1 -0.5916016   0.5185185   0.6666667
#best.2 -0.5449343   0.7407407   0.4444444
#best.3 -0.5138140   0.9629630   0.2222222
...
set.seed(1234)
regL3=cv.glmnet(matL1.1,trainL0$class3,family='binomial',standardize=T,alpha=.25)

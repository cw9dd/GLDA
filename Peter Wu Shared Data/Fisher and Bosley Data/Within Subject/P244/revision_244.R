...
#Rename
colnames(data) <- c("start","finish","energetic","enthusiastic","content","irritable","restless","worried","guilty","afraid","anhedonia","angry","hopeless","down","positive","fatigue","tension","concentrate","accepted","threatened","ruminate","reassure","procrast","avoid_people","avoid_act","hours","difficult","unsatisfy")
...
clustdat <- as.data.frame(scale(daty[, c(6,12,10,8,21,14,13,11,25,24)]))
...
summary(ICL)
blrt=mclustBootstrapLRT(clustdat, maxG=3, modelName = "VII")
blrt

#Save best model as an object
mod <- Mclust(clustdat, G = 2, modelName = "EVI") 
...
datyL0[,c(3:25,30:31,45:53,57)] <- sapply(datyL0[,c(3:25,30:31,45:53,57)], as.character)
datyL0[,c(3:25,30:31,45:53,57)] <- sapply(datyL0[,c(3:25,30:31,45:53,57)], as.numeric)
datyL1[,c(3:25,30:31,45:53,57)] <- sapply(datyL1[,c(3:25,30:31,45:53,57)], as.character)
datyL1[,c(3:25,30:31,45:53,57)] <- sapply(datyL1[,c(3:25,30:31,45:53,57)], as.numeric)
...
matL1.1 = data.matrix(cbind(trainL1[,c(3:25)],trainL0[,c(33:39,42:53)]))
matL1.2 = data.matrix(cbind(testL1[,c(3:25)],testL0[,c(33:39,42:53)]))
#Psychosocial variables + time variables + uncertainty
matL2.1 = data.matrix(cbind(trainL1[,c(3:25)],trainL0[,c(33:39,42:53,57)]))
matL2.2 = data.matrix(cbind(testL1[,c(3:25)],testL0[,c(33:39,42:53,57)]))
...
set.seed(1234)
regL1=cv.glmnet(matL1.1,trainL0$class1,family='binomial',standardize=T,alpha=.00)
...
set.seed(1234)
regL2=cv.glmnet(matL1.1,trainL0$class2,family='binomial',standardize=T,alpha=.0)
